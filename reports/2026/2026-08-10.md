# Informe de Ciberinteligencia — 2026-08-10 (UTC)

**TLP:CLEAR**

## 1. Cabecera

- **Fecha (UTC):** 2026-08-10 07:37:45
...
**Cambio del periodo:** 237 indicadores nuevos y 1 reaparecido, y 444 caídos.

Intervalo real cubierto: difiere entre fuentes — 24.5 h en `cisa-kev`, 24.5 h en `threatfox`.

## 3. Juicios clave

- **56 de las 75 familias observadas** no tienen entrada en ATT&CK. Es **posible** que esa parte del panorama activo corresponda a crimeware commodity, que el catálogo describe peor que el instrumental dirigido. *(confianza: media — medición propia sobre un catálogo con sesgo de cobertura conocido)*
...
*Las familias se observan en la ventana de 5 días que termina en 2026-08-10 07:37 UTC. La variación compara ese agregado con el de la ejecución anterior.*
...
De las **75 familias observadas**, **19** tienen entrada en ATT&CK. El porcentaje de cada técnica es la proporción de familias observadas cuyo mapeo la incluye, sobre el total de **75 familias observadas**.

**Los porcentajes no suman 100:** una familia emplea varias técnicas.

| Técnica | Familias | Proporción |
|---|---|---|
| `T1105` Ingress Tool Transfer | 14 de las 75 familias observadas | 18% |
| `T1113` Screen Capture | 13 de las 75 familias observadas | 17% |
| `T1057` Process Discovery | 11 de las 75 familias observadas | 14% |
| `T1016` System Network Configuration Discovery | 11 de las 75 familias observadas | 14% |
| `T1082` System Information Discovery | 10 de las 75 familias observadas | 13% |
| `T1083` File and Directory Discovery | 9 de las 75 familias observadas | 12% |
| `T1033` System Owner/User Discovery | 9 de las 75 familias observadas | 12% |
| `T1112` Modify Registry | 8 de las 75 familias observadas | 10% |
| `T1106` Native API | 8 de las 75 familias observadas | 10% |
| `T1125` Video Capture | 7 de las 75 familias observadas | 9% |
...
| `vwpkjni[.]spareing[.]com` | `domain-name` | `threatfox` | 100 | ClearFake | — |
...
| `threatfox` | correcta | 2816 | 0 | 0 | sí |
...
- `familia_sin_entrada`: 56 de 75 familias

**Nivel indicador** — denominador: **2816 indicadores de ThreatFox**.

- `sin_atribucion`: 296 de 2816
